...
export const getAll = asyncHandler(async (req, res) => {
  ...
  if (site_id) {
    ...
    if (Array.isArray(nativeSpecies)) {
      orConditions.push({
        _id: {
          $in: nativeSpecies,
        },
  ...
  let species = await Species.find(query)
  ...
  if (lang !== "en") {
    species = await translateData(species, ["name", "description"], lang);
  }
...
export const getById = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res
      .status(400)
      .json({ status: false, message: "Invalid Species ID format" });
  ...
  if (!species) {
    return res
      .status(404)
      .json({ status: false, message: "Species not found" });
  }
...
//     return res
//       .status(400)
//       .json({ status: false, message: "species_image is required" });
//   }
...
export const update = asyncHandler(async (req, res) => {
  ...
  console.log("UPDATE SPECIES DATA =====", data);

  if (!req.file && explicitImageDeletion) {
    return res.status(400).json({
      status: false,
      message: "species_image is required",
    });
  }
  ...
  let updatedSpecies = await Species.findByIdAndUpdate(
    id,
...
export const remove = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ status: true, message: "Species deleted successfully" });
});
